...
def emailLogin(): # returns the driver for sendEmail function to use
    ...
    username.send_keys(BOT EMAIL) # this is the email address of the bot. Using protonmail
    sleep(1)
...
def getAppointmentPage(driver):
    ...
    driver.get("https://www27.state.nj.us/tc/NJ_SelectOptions.do?method=nextLocation&domObject=appointment&domProperty=select_location&LocationId=20&LocationName=Wayne+%28Route+46%29&reschedule=false&cmd=donext&process=driver_testing_client&process2=driver_testing_client&entry=&level=&urlstring=njmvcdtclient&step=Select+Location&locationSearchType=1&location_zip=07456&location_countyId=1&location_city=&location_location=")
    sleep(1)
